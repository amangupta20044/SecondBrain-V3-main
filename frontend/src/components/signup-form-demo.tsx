...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.post(ApiRoutes.signup, { username, email, password });

      if (res.status === 200 || res.status === 201) {
        navigate("/login");
      } else {
        newErrors.email = "Signup failed. Please try again.";
        setErrors(newErrors);
      }
    } catch (error: any) {
      const serverMsg = error.response?.data?.message || "Signup failed. Email/Username may already exist.";
