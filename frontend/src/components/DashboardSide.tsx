...
const Dashboard = ({
  ...
  const getVidInfo = async (
    ...
    try {
      ...
      let rawTitle = data.title || `YouTube Video (${vidId})`;
      const cleanTitle = rawTitle.replace(/^(#\w+\s*\|?\s*)+/gi, "").trim() || rawTitle;
      const description = data.author_name ? `YouTube video by ${data.author_name}` : "";
  ...
  const ImportLinkSubmit = async (e: React.FormEvent) => {
    ...
    if (youtubeVidId) {
      ...
      setFormError({});
      setImportLink("");
    } else if (isValidURL(importLink)) {
      const userdata = JSON.parse(localStorage.getItem("user") || "{}");
      const userId = userdata?.id;
      const tagId = addingFixedTags("link");
      const alltagId: string[] = [];
      if (tagId) {
        alltagId[0] = tagId;
      }
      ...
      try {
        const urlObj = new URL(importLink.startsWith("http") ? importLink : `https://${importLink}`);
        hostName = urlObj.hostname;
      } catch (e) {}

      const linkTitle = title.trim() || `Bookmark: ${hostName}`;
      const linkDescription = description.trim() || `Saved web link (${hostName})`;
